...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                if (workinfo.workPicture == null)
                    sql = "update workinfo set workIntroduction=@workIntroduction where workId=@worId";
                return cn.Execute(sql, workinfo);
            }
        }
        ...
        public int Delete(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "delete from workinfo wrhere workid=@id";
        ...
        public int UpdateRecommend(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set recommend=@recommend,recommendTime=@recommendTime where workId=@wordId";
                return cn.Execute(sql, workinfo);
            }
        }
